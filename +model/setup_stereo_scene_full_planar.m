function [Rs, ts, projsh, lprojs, lpprojsh, pts3d, lpts3d] = setup_stereo_scene_full_planar(tlen, rotrad, pix_noise, fpix, N, llen_var)
...
%     end
    ...
    plane_pt = box_center;
    plane_norm = randn(3,1);
    plane_norm = plane_norm/norm(plane_norm);
        
    projsh = zeros(2, N, 4);
    for i = 1:N                
        [pt3d, projs] = generate_suitable_point(Rwc, t1wc, Rs, ts, box_side, pix_noise/fpix, plane_pt, plane_norm);
        pts3d(:, i) = pt3d;
        projsh(1, i, :) = projs(1,:);
        projsh(2, i, :) = projs(2,:);
        projsh(3, i, :) = 1;                                                   
...
%     end    
    ...
    for i = 1:N        
        [lpt3d, lprojs] = generate_suitable_line(Rwc, t1wc, Rs, ts, box_side, pix_noise/fpix, llen_var, plane_pt, plane_norm);
        lpts3d(:, i) = lpt3d;
...
function [lpt3d, projs] = generate_suitable_line(Rwc, t1wc, Rs, ts, box_side, pix_noise, llen_var, plane_pt, plane_norm)
    gend = 0;
    it = 1;
    while (~gend)
        gend = 1;
        ...
        pt3d_c = pt3d_c-(plane_norm'*(pt3d_c-plane_pt))*plane_norm;
        
        ldir = randn(3,1);
        
        ldir = ldir-(plane_norm'*(ldir))*plane_norm;
...
function [pt3d, projs] = generate_suitable_point(Rwc, t1wc, Rs, ts, box_side, pix_noise, plane_pt, plane_norm)
    gend = 0;
    it = 1;
    while (~gend)
        gend = 1;
        Xw = util.xrand(3,1,[-box_side/2 box_side/2]);        
        pt3d = Rwc*Xw + t1wc;     
        pt3d = pt3d-(plane_norm'*(pt3d-plane_pt))*plane_norm;
        projs = zeros(2, 4);
        for cam_i = 1:4
