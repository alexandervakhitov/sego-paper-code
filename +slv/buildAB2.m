...
function A = build_mat_for_pt(ip_num, vis_p_row, R2, t2, t2s, shift_3d, projs, t3e_dir)
    if (ip_num == 1)
        p3d1_c1 = slv.resolve_point(eye(3), zeros(3, 1), R2, t2, [projs(:, 1, 1); 1], [projs(:, 1, 2); 1]);
        if (length(p3d1_c1 ) == 0)
            A = [];
            return;
        end
        ...
        p3d1_c1 = slv.resolve_point(eye(3), zeros(3, 1), R2, t2s, [projs(:, 1, 3); 1], [projs(:, 1, 4); 1]);
        if (length(p3d1_c1 ) == 0)
            A = [];
            return;
        end
...
function B = build_mat_for_ln(ip_num, vis_l_row, R2, t2, t2s, shift_3d, lprojs, t3e_dir, R3, t3)
    ...
    [X1, X2] = slv.resolve_line(eye(3), zeros(3, 1), R2, t2p, lproj1, lproj2);
    if (length(X1) < 3)
        B = [];
        return;
    end
