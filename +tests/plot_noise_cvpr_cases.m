function plot_noise_cvpr_cases
    ...
    %load('noise_cvpr_l1.mat');
    load('noise_cvpr_1e5.mat');
...
%     for si = 1:9
%         for mi= 1:5
%             fprintf('%f ', median(ml(mi).mean_time(:, si)));
%         end
%         fprintf('\n');
%     end
...
%         'LineStyle', method_list(i).linestyle, ...
        ...
        if (strcmp(ml(mi).id , 'OPnPL') || strcmp(ml(mi).id , 'BA'))
            ml(mi).marker = '.';
            ml(mi).color = 'k';
    ...
    for si = 1:9                
            ...
            shift = si;
            subplot('Position', [0.21+0.33*(col_ind-1), 0.15+0.3*(3-row_ind),  0.12 0.2]);

            ws = ones(length(ms), 1);
            yrange = [0 40];            
            if (si == 1)
                [mnames1, p] = util.xdraw_main(Xarg,yrange,ms,'med_r',[''],XLabelc,'Rotation', ws);            
            else
                [mnames1, p] = util.xdraw_main(Xarg,yrange,ms,'med_r',[''],XLabelc,'Rotation', ws);                        %Median Rotation
%                 set(gca,'YTickLabel','')            
            end
            set(gca,'FontSize',10);
            subplot('Position', [0.05+0.33*(col_ind-1), 0.15+0.3*(3-row_ind),  0.12 0.2]);
            %, 'Position', [0.4, 1-0.3*(si) 0.2 0.2]
            ws = ones(length(ms), 1);
            yrange = [0 210];            
            if (si == 1)
                [mnames2, p] = util.xdraw_main(Xarg,yrange,ms,'med_t',[case_labels{si}, ''],XLabelc,'Translation', ws);%Median Translation

            else
                [mnames2, p] = util.xdraw_main(Xarg,yrange,ms,'med_t',[case_labels{si}, ''],XLabelc,'Translation', ws);
%                 set(gca,'YTickLabel','')
            end
            set(gca,'FontSize',10);
...
%                 set(gca,'FontSize',12);
        ...
        print(h,['figs/var_noise_cvpr_cases' '.pdf'],'-dpdf','-r0')
%         pause;    
end